...
class PopMoovieCard extends Component {
    ...
    componentDidMount(){
        ...
        if(storageAArray !== null){
          let estaEnElArray = storageAArray.includes(this.props.peliculaCartel.id)
          if(estaEnElArray){
            this.setState({
              esFavorito: true
            })
        ...
        return (
                ...
                </div>
            {
              this.state.esFavorito ?
               <button onClick={()=> this.sacarFav(this.props.peliculaCartel.id)}> Sacar de Favs</button>
              :
                <button onClick={()=> this.anhadirFav(this.props.peliculaCartel.id)}>Añadir a Favs</button>
            }
